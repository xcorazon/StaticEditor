package  pr1.Shapes {
	...
	public class LinearDimensionCreatorY extends Sprite {
		// события в объекте
		public static const CREATE_CANCEL:String = "Cancel creation of dimension Y";
		public static const CREATE_DONE:String = "Done creation of dimension Y";
		...
		private var razmer:LinearDimensionYContainer = null;
		...
		public function LinearDimensionCreatorY(parent:*, segments:Array) {
			...
			this.linearDimension = new LinearDimension(new Point(0,0), new Point(0,-15), -20, Math.PI/2, 0);
		...
		private function onMouseMove(e:MouseEvent){
			switch(doNow){
				...
				private function doSnapToSecondPoint(e:MouseEvent){
					...
					linearDimension = new LinearDimension(firstCoordinate, p, -20, Math.PI/2, 0);
				...
				private function doMoveHeight(e:MouseEvent){
					...
					//находим высоту
					height = firstCoordinate.x - cursorPosition.x;
					razmerHeight = height;
					
					linearDimension = new LinearDimension(firstCoordinate, secondCoordinate, height, Math.PI/2, 0);
					button_over = new LinearDimension(firstCoordinate, secondCoordinate, height, Math.PI/2, 0xff);
					button_up = new LinearDimension(firstCoordinate, secondCoordinate, height, Math.PI/2, 0);
		...
		private function doSelectHeight(e:MouseEvent){
			...
			dialogWnd.addEventListener(EditWindow.CANCEL_EDIT, onCancelEditInDialogWindow);
						...
						private function onEndEditInDialogWindow(e:Event){
							...
							dispatchEvent(new Event(LinearDimensionCreatorY.CREATE_DONE));
						}
		
								private function doCreateLinearDimension(){
									var p:Point;
									var angle:Number;
									razmer = new LinearDimensionYContainer(parent1,button_up, button_over, button_down, button_hit,
																  razmerName);
						...
						private function onCancelEditInDialogWindow(e:Event){
							...
							dialogWnd = null;
							dispatchEvent(new Event(LinearDimensionCreatorY.CREATE_CANCEL));
						}
		
		public function get result():LinearDimensionYContainer{
			return razmer;
		}
